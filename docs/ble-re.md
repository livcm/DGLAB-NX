...
### 当前状态（2026-09-26）：正式入口跑通，软上限是唯一的输出闸门
...
| 输出闸门 | ⚠️ **BF 软上限**：0 就是设备不会有任何输出。它有两个入口——`BLE_START` 带初值，运行中改走 `BLE_LIMIT`（会话漏了这个动作时，页面上的 `↑` 只改数字，设备纹丝不动） |
| 回读（B1/电量/读值） | ❌ 按不可得收束（见上一节），BLE 模式因此是**开环** |

**没跑到的只有一条**：软上限 > 0 时用玩法页驱动设备的实机确认（也就是"体感/触屏在正式
会话路径上真的让设备输出"）。2026-09-26 那轮的上限是 0，所以只有写入、没有输出。
