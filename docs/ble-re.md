...
**（已被下面的「当前状态（2026-09-26）」取代：同日把 BLE 包成了正式入口，并定了"软上限
是唯一输出闸门"这一条。本节保留当时的证据链与配方。）**
...
1. **回读方向已查到底、按"不可得"处理**（结论与证据见下面的「连接所有权在服务层是封的」）：
   不再在订阅、事件队列或配对方向上投入；
2. **BLE 模式按开环设计落地**：把已经在实机上跑通的"波形 + 强度 + 软上限"接到 NRO 的玩法
   （体感/触屏），替换现在不成功的 btdev 路径；UI 不显示真实强度/电量，不做基于设备当前值的
   相对加减（开环跟踪只能表达"我请求过什么"）；
3. **补丁纳入发布产物**（用户已定口径：BLE 直连仅在安装该 exefs 补丁时可用）。
...
### 连接所有权在服务层是封的（2026-09-25 深夜，静态分析收束）

三条线索都走完了，结论是：**第三方进程无法让这条 BLE 连接归自己的 `client_if`**。

1. **连接要槽位**：btdrv `ConnectGattServer`（cmd 65）不理会调用者传的 `client_if`，而是按地址
   去查管理器的**设备槽位表**（10 个槽位、步长 `0x332`、`{有效标志, 地址[6], client_if}`），
   查不到就回 `Bluetooth/0x1806`（见上面的「连接归谁」）；
2. **槽位只有一条写入口，而且不可达**：唯一写表的是内部消息 `0x18 / kind == 1` →
   `FUN_0002e400` → `FUN_000079e0`（占槽位）/ `FUN_000080b0`（写 client_if）；这条链的起点
   `FUN_00008820` 在 `bluetooth` 模块里**没有任何引用**——全模块没有 `bl 0x8820`、没有
   `adrp/add` 组合算出 0x8820、也没有 8 字节指针指向它（三种方式都扫过）；
3. **btm 的连接不经过 cmd 62/65**：btm 服务命令 35（`BleConnect`）走的是它自己的管理器
   （btm 模块内虚表 `+0x130`）；btm 面向 btdrv 的客户端跳板表（113 项，数据地址 `0x8ce18`）
   覆盖的命令是 **66–102、128–155、256–258**，而 `23`（`TriggerConnection`）、
   `62`（`RegisterGattClient`）、`65`（`ConnectGattServer`）**都不在其中**，这三个跳板在模块里
   也没有任何数据表引用；
4. 连带确认：btdrv **cmd 54 = `SetBleAdvertiseParameter`**（广播参数 `addr + min + max`），
   与客户端认领无关；`docs/ble-re.md` 早期那条"cmd 54 待查"可以划掉。

也就是说：**能连上的那条路（btm）是模块私有的，服务层没有把它暴露出来**；第三方唯一可用的
btdrv 连接接口恰好是被槽位/激活闸门挡住的那些。这不是"还没试对命令"，而是服务 API 层面的封闭。
除非固件更新或上游出现新资料，**回读方向（B1、电量、读到的值）对第三方进程不可得**。

因此 BLE 模式按**开环**设计：波形、强度、软上限（设备侧钳制）照用，不依赖任何回包；UI 不显示
真实强度/电量，也不做基于设备当前值的相对加减。相关实机证据（写入确实生效、回包全哑、绑定与
配对都是设备侧强制）见 `docs/ble-poc.md` 与 `docs/dglab-protocol.md`。

### 当前状态（2026-09-26）：正式入口跑通，"在输出"靠波形、闸门是两个通道强度上限

BLE 不再是"只有探针能跑"：NRO 的 `bluetooth (direct)` 页（IPC 的 `BLE_START` / `BLE_STOP` /
`BLE_STATUS` / `BLE_LIMIT`，接口 0.2.2）已经把整条路包成两段——**驱动级探针**（唯一能
`InitializeBle` / `EnableBle` 的地方）把 BLE 栈打开，随后 **BLE 会话**连接、订阅并流式写；
玩法页（体感/触屏）照旧发它们的强度与波形，sysmodule 在会话激活时把 `NET_SEND` /
`NET_WAVEFORM` 路由到本地协议层，游戏代码一行没改。

| 环节 | 状态（2026-09-26） |
| --- | --- |
| 设备发现 | ✅ 驱动级扫描按 AD 判据认出设备并写 `config/dglab-ble-address.txt`，客户端零配置 |
| 连接 | ✅ `btmInitialize` → `btmBleConnect` → `connected handle=4`（漏掉 `btmInitialize` 时会得到 `Kernel/114` `0xE401`，见错误码表） |
| GATT 表 | ✅ 7 个服务，`0x180C`（写 `0x150A` handle 19、通知 `0x150B` handle 16） |
| 通知订阅 | ⚠️ `RegisterNotification rc=0`，仍然收不到任何事件（同上，回读不可得） |
| 写入 | ✅ 会话路径实机 `writes=172` 全 `rc=0`（2026-09-26）；"真的到设备"的硬件证据仍来自 2026-09-25 15:58 的探针反应测试 |
| 输出闸门 | ⚠️ **BF 的两个通道强度上限**：某一通道的上限是 0 就是那一通道不会有任何输出。上限是**一对**变量（A 一个、B 一个，接口 0.2.3 起 `BLE_START` / `BLE_LIMIT` 都带两个数），和"强度"是两组不同的变量。设置页（`advanced (motion)`）里的两行就是它们，默认 100 |
| 输出状态（灯） | ✅ 靠**波形**成立，不靠强度：手机 App 实测（用户，2026-09-26）**上限 0 + 强度 0 + 打开输出，设备上的灯照样闪**。所以会话现在自己给两个通道装一段默认波形（100ms、波形强度 100，与 Socket 页测试键同形状），强度为 0 时灯也闪；不播波形（B0 波形槽全零）时设备按官方规则丢掉整通道数据，那就真的什么都看不见（2026-09-26 那轮 172 包全零波形就是这一格） |
| 回读（B1/电量/读值） | ❌ 按不可得收束（见上一节），BLE 模式因此是**开环** |

**灯闪这一条是手机给的判据**：用户 2026-09-26 在手机上试过——**上限与强度都是 0、打开
输出，设备上的灯也会闪**。上一轮主机侧的日志因此有了确切的读法：`streaming` 之后写的全是
`B00000…`（两通道波形槽全零，等于设备把两个通道的数据都丢掉），灯不闪不是"上限 0"这么简单，
而是**根本没有波形在播**。"强度只是闸门、输出靠波形"这条（2026-09-25 的结论）在设备侧的
对应现象就是这盏灯。

**没跑到的还是那一条**：上限 > 0 时用玩法页驱动设备的实机确认（"体感/触屏在正式会话路径上
真的让设备输出"）。这一版（上限拆成 A/B 两行 + 会话自带默认波形）**尚未实机验证**：卡上的
构建要重新装、重启主机，跑法见 `docs/ble-poc.md`；第一眼看的应该是"起会话后设备的灯开
始闪"，再看把强度拨上去有没有感觉。

**实机前先确认设备没绑在手机上**：设备里只要有绑定信息，第三方主机连得上、**写入会被设备
整个忽略**（2026-09-25 用户对照实验，见本文「配对支线」那节的表）。这一条不满足的话，
"灯不闪"与"没感觉"都不能归因到我们这边——先像上次那样把「设备绑定」关掉或重置设备，
再做这一轮的判断。

**其余待做**：把补丁纳入发布产物；玩法轮跑通后收口文档。

日志上的两条规矩（这一轮踩过）：会话与探针的日志只在 sysmodule 的内存 ring 里，只有 NRO
在主机上跑着时才会被抽到卡上（`bluetooth (direct)` 页 → `logs/dglab-net.log`）；**拔卡就是
关机**，ring 随之消失，所以留日志必须在同一次开机里、关主机之前完成。
