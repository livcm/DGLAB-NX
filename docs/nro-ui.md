...
| 日志子页 | Y 打开：页头 + 32 行日志（24px、行距 37px、白色），**用整幅宽**（x=80..1190，和 socket 页同一带宽——日志行本来就长，这一页没有要和内容列对齐的东西），打开时停在最新；底栏不画滚动提示（滚动条即提示）。放不下的行由 `dglabTextFitLine()` 按**真实像素宽度**裁断并补 `...`（不是按字符数切：那只能对一种字体成立）。socket 页的是 sysmodule 日志，bluetooth 页的是会话日志，**同一套绘制**（`dglabLogPageDraw()`），只有标题不同 | ↑↓ 滚动、Y 关闭、B 返回 |
