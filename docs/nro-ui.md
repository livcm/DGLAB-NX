...
| 按键图标 | 实心形状 + 字母挖空，字母 22px（墨迹≈15px），圆盘 26px | 底栏与页内提示行；字母用页面背景色挖出 |
...
| 字号 | 28 / 24 / 22 / 18 | 页头 / 行与标签 / 值 / 说明（`text.h`） |
...
- `tests/canvas` 的 `testEveryPageStaysInItsRegions` 把五屏 × 两种语言渲染一遍（外加
  菜单每个条目、服务端页四种光标位置、体感页两种状态、高级参数页首尾两项），检查
  没有任何像素落在"页头带 / 内容列 / 底栏 / 滚动条列"之外——这是旧版
  `testNothingLeavesItsPanel`（"文字不许压到面板框线"）在无边框界面里的对应物；
- 预览出图仍然是 `tests/canvas/tools/render_preview.c`，现在会用 `PREVIEW_TTF` 开出
  四个字号（`normal` / `menu` / `motion` / `advanced` / `about`）。
...
NRO 的 framebuffer 固定 `1280×720`（`nro/source/platform/framebuffer.c`），dock 下由系统的
scaler 把这张 720p 画面等比放到 1920×1080 —— 这就是需求里的"1080P 下就是 720P 等比例放大"，
不需要 NRO 自己做缩放。

代价与后续选项：

- 好处：布局只有一套坐标（720p），所有量出来的 HOS 规格（71px 行高、28/24/22/18 字号）
  原样成立；
- 代价：dock 下不是原生 1080p，文字会比原生渲染软一点。若要原生 1080p，做法是在 canvas
  层加一个 3/2 比例因子（位置乘 1.5，四套字号按 42/36/33/27px 光栅化，测量函数返回逻辑
  单位），屏幕代码不用改——但这是增强，不是现在必须的；
- **实机待确认**：dock 下 `nwindowGetDefault()` 配 1280×720 的 framebuffer 是否确实走系统
  缩放（电脑上无法验证），以及 720p 上屏后的锐度是否可接受。
...
| 体感（Joy-Con） | 页内提示行 + 5 行（连接、通道 A/B、通道 A/B 强度）+ 安全提示 note | 与连接测试相同 |
