...
| 日志子页 | Y 打开：页头 + 32 行日志（24px、行距 37px、白色），**用整幅宽**（x=80..1190，和 socket 页同一带宽——日志行本来就长，这一页没有要和内容列对齐的东西），打开时停在最新；底栏不画滚动提示（滚动条即提示）。放不下的行由 `dglabTextFitLine()` 按**真实像素宽度**裁断并补 `...`（不是按字符数切：那只能对一种字体成立）。两页**同一套绘制**（`dglabLogPageDraw()`），只有标题不同；**内容各有一条环**：socket 页拉 sysmodule 的 net 环（`NET_LOG`），bluetooth 页拉 PoC 环（`POC_LOG`），互不挤占（见下） | ↑↓ 滚动、Y 关闭、B 返回 |
...
- 日志环**每个来源一条，各 32 行**（`DGLAB_SCREEN_LOG_LINES`；`main.c` 的 `DglabLogRing`：
  `g_log_net` / `g_log_ble`，各自带 cursor 与"半行"缓冲）。sysmodule 那边本来就是两条
  （socket 服务端的 `NET_LOG` 与 BLE 会话的 `POC_LOG`），而前端早先只留一条屏上滚动缓冲：
  哪个页面最后拉过谁，那 32 行就是谁的，于是两个标题不同的日志页显示同样内容（2026-09-26 修）。
  NRO 自己写的行（`auto sleep:`、`theme:`、体感那几行 `motion …`）不属于任何传输，进 net 环；
  `ble session:`/`btm transport:` 与"BLE 启动被拒"进 BLE 环；
- 每行最多留 `DGLAB_SCREEN_LOG_LINE_LEN`（192）字节——那是 sysmodule 能交过来的最长一行，
  **屏上再按宽度裁**（`dglabTextFitLine()` + 行尾 `...`），不是按字符数提前切；
  SD 卡上的镜像文件**两来源合写一个**（`logs/dglab-net.log`：一轮测试一个文件，方便直接发回来），
  镜像里的每行都是完整的；
...
- `dglabPageClipContent()`：单栏页用，x=220..1060（菜单、motion、advanced、about）；
  日志子页是唯一的例外，它用 `dglabPageClipWide()`（x=80..1190，见上表）；
