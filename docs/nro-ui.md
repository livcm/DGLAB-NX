...
| bluetooth (direct) | 页头 + 5 行值（会话、设备、软上限、请求的强度、已发包数）+ 两段说明（开环、启动的两步）；排不下时出滚动条。**这一页还会把 sysmodule 的 PoC ring 落到 `logs/dglab-net.log`**——会话自己的日志（`ble session: …`）只在那条 ring 里，没有它就只能靠屏幕猜 | A 启动（内部先跑驱动级再连会话）、X 停止、↑↓ 调软上限（0~100，步进 1，与 socket 页一致）、B 返回（离开会自动停会话） |
