...
- 传输层会写 BF（通道强度上限与平衡参数）。**基线阶段两个上限都是 0**，设备没有任何输出；
  v20 起的**反应测试**会临时把上限抬到 20、请求强度 5、给通道 A 发一段短波形——那是
  "设备到底有没有收到我们的包"的唯一硬件判据，收尾时先把 BF 写回 0 再把两通道请求归零。
  想彻底不输出，把 `POC_BTM_TEST_STRENGTH` 设成 0（反应测试自动跳过）。
- 注意 BF 的通道强度上限**断电保存**：PoC 写完 0 之后设备会一直保持这个上限，直到官方 App 下一次
...
| **`←`（十字键左）** | 只跑驱动级探针会话（btdrv 扫描 + 广播 dump + `InitializeBle`/`EnableBle`，再用管理器给的 `client_if` 自己连一次；v28 起在连接还活着时补一次**配对探测**——收到配对请求就取消，详见 `docs/ble-re.md` 的「连接归谁」） |
| `-` | 停止会话 |
| `+` | 退出 NRO |
...
连接必须知道设备地址（BLE 没有"匿名连接"），而这个地址**不需要手写**：

- 驱动级会话的扫描窗口是唯一能看到这台设备的扫描（btm 自己的扫描从来没有结果），它会把每个
  新出现设备的广播解码出来；
- 广播里认出这台设备的判据：**本地名 `47L121000`**（AD 类型 `0x09`）或**厂商数据公司号
  `0x000A`**（AD 类型 `0xFF`，前两字节 `0A 00`）；
- 如果**配置里还没有地址**，就把该地址写进 `SD:/switch/DGLAB-NX/config/dglab-ble-address.txt`
  ——正是 NRO 每次 START 前都会读的那个文件，所以**同一次一键序列的第二段（btm 会话）会直接
  用上它**，用户什么都不用配；
- 已经配了地址就**不覆盖**（留给"指定某台设备"的用法）。想重新自动发现，把这个文件删掉再跑一轮。
...
每次会话开头都会打印 `poc build: ble_poc v28 (pairing probe on a live connection)`：拿到日志先看
...
3. `dglabCoyoteV3SessionOnConnected()` 立刻写 **BF**（7 字节，通道强度上限 + 平衡参数）；
...
6. **拨轮阶段**（v24 起）：把上限写到 10、给 A 通道装一段温和波形（峰值 30），然后
   **我们自己一个强度包都不发**，等 15 秒——请这时**拨设备自己的 A 通道拨轮**。这是唯一
   ...
   才改强度"；
7. **反应测试**（v20 起）：把两个上限写到 20、给 A 通道装一段"慢升慢降"的波形、请求
...
8. 收尾：上限写回 0（BF 立即生效，等于先把设备的能力掐掉）、清波形、两通道请求绝对零、
...
波形峰值 = 60，也就是设备最大输出的百分之几；上限每次连接由官方 App 重写，所以不会把设备
...
### 第五十三次实机（2026-09-26）：正式入口连上了，卡在通道强度上限 0；环形日志只有内存那一份
...
| 09-26 01:58 | v93 | `btmInitialize rc=0` → `BleConnect rc=0` → `connected handle=4` → `0x180C` + `write=19 notify=16` → `streaming, soft limit 0` → `writes=172 notify=0 b1=0` | 连接/订阅/写入全通，**但 BF 写的是 `0000`**：上限默认 0 而且只在 `BLE_START` 那一刻被读一次，之后按 `↑` 只动屏幕（`109881a` 修） |
...
所以"没有输出"跟连接、GATT、写入都无关：**通道强度上限是设备侧的输出闸门，它是 0 时设备
不会输出任何东西**，而那一轮的上限就是 0。

**但上限 0 不是全部**（同一晚，用户手机实测）：在手机 App 里**上限与强度都是 0、打开输出
时，设备上的灯照样闪**。对照这一轮的日志——`streaming` 之后写出去的全是
`B000000000000000000000000000000000000000`，两通道的波形槽**全零**，按官方规则
"某通道 4 组里只要有一个值超出有效范围就放弃该通道全部数据"，设备等于什么都没在播：
**灯不闪、强度加上去也没有东西可放大**。这条把"强度只是闸门、输出靠波形"（2026-09-25 的
结论）在设备侧对上了：灯 = 有波形在播。
