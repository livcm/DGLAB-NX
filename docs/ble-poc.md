...
| **`←`（十字键左）** | 只跑驱动级探针会话（btdrv 扫描 + 广播 dump + `InitializeBle`/`EnableBle`；v25 起还会写一条**配对设备记录**、然后用管理器给的 `client_if` 自己连一次，见 `docs/ble-re.md` 的「连接归谁」） |
| `-` | 停止会话 |
| `+` | 退出 NRO |
...
每次会话开头都会打印 `poc build: ble_poc v25 (paired-device record, then our own client_if)`：拿到日志先看
