...
| `StickR`（把右摇杆按下去）或**空闲屏 `B`** | **一键序列（v18 起）**：先起驱动级探针会话（它把 BLE 栈打开），该会话结束后自动起 **base `btm` 探针会话**（扫两遍 general / smart-device → 连配置地址 → GATT 表 → 传输层 BF/B0 + 等 B1 → v20 起的反应测试：小强度 + 波形）。**btm 探针曾导致 `btm` 崩溃，现在是手动触发、跑完必须重启**，见下 |
...
每次会话开头都会打印 `poc build: ble_poc v20 (reaction test: waveform + tiny strength)`：拿到日志先看
...
1. `btLeClientRegisterNotification(handle, 0x180C, 0x150B)` 订阅通知，回读一次 CCCD，
   然后**手工写一次 CCCD**（`0x2902 = 0100`）：`RegisterNotification` 的 `rc=0` 只代表
   "受理"，手工写才知道订阅有没有真的发出去；
...
6. **反应测试**（v20 起）：把两个软上限写到 20、给 A 通道装一段"慢升慢降"的波形、请求
