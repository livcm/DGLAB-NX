...
| `Left` | 手动运行一次 btdrv 驱动级探针（不再自动跑，理由见下） |
| `Right` | 运行一次 btdrv 身份探针（读本机名称/MAC/信道图 + GATT 注册序列，见下） |
| `StickL` | 对照扫描：用常见厂商 ID（Apple/Microsoft/Samsung 轮换）扫描，验证扫描机制本身 |
| `-` | 停止 PoC（清理并退出） |
| `+` | 退出 NRO |
...
注册不是同一条；它这次仍然被拒（`0x5568F`），而且配的地址 `EA:A8:AC:22:2C:18` 是随机静态
地址，本来就可能已经变了，正式测之前要用手机扫一遍确认。
...
1. **地址是否还有效**：`EA:A8:AC:22:2C:18` 是随机静态地址，换一次开机就可能变。用手机扫一遍
   写进 `config/dglab-ble-address.txt`。
2. **扫描结果里地址是不是落在别的偏移**：固件的请求形状已经证明和 libnx 不一致，事件载荷
   布局同样可疑。排水现在会把 ScanResult 的前 32 字节整段打出来，并在整块里搜索配置的地址、
   命中时打印偏移；按 `Left` 跑一次驱动级扫描就能看到。
