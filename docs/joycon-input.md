...
- **静止时的策略**：死区内先继续发槽位，让强度沿释放曲线降到 0；连续 250ms 都低于死区
  阈值就**停止上传**，队列自然放空、输出停下、流量归零。再次超过进入阈值时立刻恢复上传
  （第一批立即下发，服务端立即补流，所以起播只差一帧加网络）。
  为什么不一直发 0：静止也发的话 IPC/WebSocket 会一直跑（40 槽/秒 ≈ 每秒 2~3 条 pulse
  命令），App 的日志会被无意义的 `tx` 刷满；停发省掉这些唤醒，代价只是重新起播的几十
  毫秒，而释放曲线已经把它衰减到 0，听感上不会"咔哒"；
