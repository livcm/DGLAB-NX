...
**句柄会过期，所以每次进入玩法都重新取一套，静默太久也重新取一套。** 句柄是按 "(玩家 1,
风格)" 拿到的，而风格会随使用方式变化：Joy-Con 插回主机、按同步键关掉再开、换成单只模式，
系统都会重新分配。拿着旧句柄继续轮询的表现就是"一直未连接、通道也不出声"——**实机反馈
（2026-09-17）**里左路正是这样（日志里 `#0 states 2, samples 0, connected 0` 一直不变）。现在：

- `dglabJoyconStart()` 每次进入玩法都重新取一套（不再"已经在跑就直接返回"）；
- 某一侧连续 `RESCAN_AFTER_QUIET_POLLS`(120) 次轮询没有任何读数，就重新取一套，并把次数记在
  日志的 `rescans` 里——所以"自动恢复"有没有发生，日志里看得见；
- 日志行里还有当前生效的 npad 风格，形如 `motion styles 0x00000004+joydual`（位含义：
  `1 fullkey`、`2 handheld`、`4 joydual`、`8 joyleft`、`16 joyright`）。插回主机时系统通常会切到
  `handheld`，那时一对 Joy-Con 的句柄本来就取不到数据——**这是判断"未连接"到底是对的还是句柄
  过期了的唯一实机依据**。
...
**唯一依据是"读数有没有来"**，句柄自称的 `IsConnected` 不算（`nro/source/motion/motion_feed.c`
...
这条规则是两轮实机反馈磨出来的，两次都是同一个字段的两个极端：

- **2026-09-16**：一对拆下的 Joy-Con、波形输出正常，两行却都显示未连接——旧实现是"最后一个
  回答的句柄的最后一条读数说了算"，一个占位读数就把整侧判死；
- **2026-09-17**：反过来，一旦进了"挥动中"就再也回不到"未连接"（关掉、插回主机都不变）——
  这次是因为 `IsConnected` 位仍然为真而读数已经不来，而规则相信了那个位。

所以现在的实现只认读数：活的传感器每帧都把 LIFO 填满（实测每次轮询 16 条），安静下来就是
不在了。用**轮询次数**而不是毫秒计数，是为了让一次卡帧/一次 IPC 停顿看起来只是"一次轮询"。

**单只风格的回退句柄还要看风格脸色。** 一对 Joy-Con 插回主机时系统会切到 `handheld` 风格：
一对的句柄开始回占位读数，而单只风格句柄（同一只物理手柄的另一个名字）可能仍在给数据——"一
旦挥动中就永远不显示未连接"的另一半原因就在这里。现在：

- 每一侧**永远轮询 `NpadJoyDual` 的句柄**（它是这个玩法的正主，读数就是命）；
- 只有当**当前生效的 npad 风格**里包含该侧的单只风格时，才允许回退到单只句柄：

      styles = hidGetNpadStyleSet(HidNpadIdType_No1)
      1 = fullkey   2 = handheld   4 = joydual   8 = joyleft   16 = joyright

- `styles == 0` 表示"系统没告诉我们"（例如两只都没连上），这时按老办法两个句柄都试。

日志里 `motion styles 0x…` 就是这句话的值，`#n style off` 表示某个回退句柄被闸掉，
`rescans N` 是因长时间没有读数而重新取一套句柄的次数（实机日志里出现过 18 和 46，说明这条
恢复路径确实在用）。
...
**诊断行本身踩过一次坑**：给描述加了 `quiet`、`#n` 之后，行变长了，而 `main.c` 里的
`left[96]` / `right[96]` 缓冲没跟着改，于是日志里那行被从中间截断（`connecte | right: …`）——
诊断信息自己变成噪音。加字段时记得连缓冲一起量一遍。
