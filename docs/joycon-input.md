...
**单只风格的句柄只在取不到一对句柄时才用。** "一旦挥动中就永远显示未连接"的另一半原因，是
一对 Joy-Con 插回主机后一对句柄开始回占位读数，而单只风格句柄（同一只物理手柄的另一个名字）
还在给数据，那一侧就永远活着。现在一对句柄可用时**只用它**，单只句柄只作为"这台机器根本取不
到一对句柄"时的兜底（见上面第 3 条：按风格判断这条路因为 API 返回 0 已经删掉）。
