...
**句柄只在"进入玩法"和"手动重新扫描"时取，而且两侧永远来自同一次调用。** 句柄是按
"(玩家 1, 风格)" 拿到的，风格会随使用方式变化（插回主机、关掉再开、切成单只），所以旧句柄会
失效。2026-09-17 的实机反馈把这条路走成了三个教训：

1. **两侧必须由同一次 `hidGetSixAxisSensorHandles(pair, 2, …, NpadJoyDual)` 给出**。之前是
   每侧各取一次，当只有一只 Joy-Con 拆下来时，某一次重新取句柄就可能把那一侧绑到**另一只手柄**
   上——用户看到的现象正是"右手柄的波形值跑到左手柄那一行、右手柄那一行再跟一个略低的值，然后
   两行定住"。现在 `acquireAll()` 一次调用把 pair[0]/pair[1] 分给左右，不可能再串。
2. **不要按侧"静默太久就重新取句柄"**。一侧静默是常态（那只手柄被关掉或插回主机了），据此重新
   取句柄就是上面串线的来源（实机日志里 `rescans 24` / `158` 就是这种churn）。现在只有**两侧
   同时**静默 `RESCAN_AFTER_QUIET_POLLS`(120，约 2 秒)才重新取一套（`rescans` 计数仍在日志里）。
3. **`hidGetNpadStyleSet(HidNpadIdType_No1)` 在这个 applet 里始终返回 0**（实机日志：
   两只 Joy-Con 都在正常出数据时也是 `motion styles 0x00000000`），所以"按生效风格决定用哪个
   句柄"这条路走不通、已经删掉。取而代之的规则更简单：**一对风格可用时只用它的句柄**；单只
   风格的句柄只在"连一对句柄都取不到"的机器上才当兜底。这样插回主机、关掉手柄时那一侧就没有
   第二个句柄能把它"救活"，该显示未连接就显示未连接。

取句柄的时机因此是：

- **进入玩法时自动取一次**（`dglabJoyconStart()`）；
- **按 `Y`（页面底栏「重新扫描手柄」）再取一次**——手柄关掉再打开、或从主机上拔下来之后，
  按这个就能把那一侧救回来，不用退出重进玩法。

另外，**"手柄不再有读数"现在也会让动作结束**：`motion_feed` 的 `moving` 以前只在采样进来时
才更新，所以手柄一走，行上会停成"挥动中 / 波形值 0 / 100ms"（实机反馈里的"定住"就是它）。
现在释放曲线走完、窗口里又没有任何采样时，`moving` 复位成静止。
