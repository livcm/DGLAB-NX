...
2026-09-15 的实机日志里，App 成功绑定、我们的 `tx command …` 发了三十多条，但
**App 一条消息都没回过**（连心跳都没有：`has app … has been quiet for 90 s` 是靠任何
入站消息刷新的，说明真的完全静默）。也就是说 App 收得到、却不执行也不回报。

在确认"App 的设备连接本身是通的"之前，剩下的可能只有指令形状与信封路由字段。所以加了
一个临时开关：`ZR` 在 0~5 之间切换，`ZL`/`X` 用当前形状发送，日志会写
`tx command … variant N`。

| 变体 | 通道强度 | 波形 | 备注 |
| --- | --- | --- | --- |
| 0 | `strength-1+2+N` | `pulse-A` | 参考实现格式（默认） |
| 1 | `strength-0+2+N` | `pulse-1` | 通道号从 0 开始 |
| 2 | `strength-A+2+N` | `pulse-A` | 通道用字母 |
| 3 | `strength-A+1+N` | `pulse-A` | 字母 + 相对增加 |
| 4 | `strength-1+1+N` | `pulse-1` | 数字 + 相对增加 |
| 5 | 同变体 0 | 同变体 0 | 但信封的 `clientId`/`targetId` 对调 |

找到能用的形状后，应当把它固化为默认并删掉这个开关。
