...
波形本身由手机 App 负责（App 里选/播放波形），Switch 侧的职责是**把事件映射成强度
控制信号**推过去：

    游戏事件 / 手柄传感器 / NRO 参数
              ↓  事件源自己决定曲线（受击 +N、随时间衰减、连击加成…）
        IPC: NET_SEND（SetStrength / IncreaseStrength / DecreaseStrength / Clear）
              ↓
        Sysmodule → strength-<ch>+<mode>+<value> → App → 郊狼

因此本项目**不在 Switch 上合成波形**：`pulse` 只在测试按键里用一次，用来验证链路。
`strength` 的三个模式（设置 / 增加 / 减少）就是给事件源用的原语——尤其是相对增减，
这样事件源不必自己记账当前强度。

两点限制要记住：

1. App 是单向的，不上报强度，所以我们只能"本地记账"（我们设多少就认为是多少）；
