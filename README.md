...
| `overlay/` | 未实现 |
| `mods/` | 未实现 |
| BLE 模式（sysmodule 直连设备） | 入口已实机跑通（`bluetooth (direct)` 页 + `BLE_*`）：连接 + GATT 表 + 流式写入（`writes=172` 全 `rc=0`），玩法页的强度/波形原样走 BLE；**但依赖 exefs 补丁、通道强度上限为 0 时那一通道不输出、且通知路径不可得**（设备不回 B1）。当前状态与下一步见 `docs/ble-re.md` 的「当前状态（2026-09-26）」；实测记录见 `docs/ble-poc.md` |
...
其中 **bluetooth (direct)** 是 BLE 模式的正式入口（起会话、看状态、拨强度、看日志；玩法页
照旧用，数据会自动走 sysmodule 的 BLE 会话；见 `docs/ble-poc.md` 与 `docs/ipc.md` 的
`BLE_*`）。`↑`/`↓` 拨 A 通道的**强度**、`←`/`→` 拨 B 通道的强度（和 Socket、玩法页完全
一样，按住连发）；两行强度显示的是 **`值/上限`**，右边的上限就是拨盘停住的地方，而在这一页
它始终取「高级参数（体感）」页里那对**通道强度上限**（默认 100；改成 0 就是"那一通道不会
有任何输出"——连上却一点感觉都没有，先看这两个数）。这一页只有五行、排得进一屏，不放说明
文字。会话跑起来后两个通道会一直播一段默认波形，所以强度、上限都是 0 时设备上的灯也会闪；
`Y` 打开这一页自己的日志（和 Socket 页的日志页同一套布局），`A` 启动会话、`X` 停掉，
`B` 返回（返回时会话会一起停）。
...
| `↑` / `↓` | 调整通道 A 强度（0~该通道上限，步进 1，按住连续调整，改完立刻发给 App） |
| `→` / `←` | 调整通道 B 强度（同上） |

两个通道的强度各自独立，都从 0 开始；调整即刻发送给 App，所以没有单独的"发送强度"
按键。服务端页、体感页、触屏页和蓝牙页的强度行都写成 **`值/上限`**（例如 `20/80`）：
左边的数是拨盘当前的值，右边的数是它停住的地方——App 已经上报过自己的上限时用 App 的
（Socket、体感、触屏三页，那才是真正在钳制的那个），还没有上报时用「高级参数（体感）」页里
的通道强度上限（蓝牙页始终用后者）。左栏「App 上报」那行仍然显示 App 报过来的原始值。
