...
一样，按住连发）；页面上的两条**通道强度上限**是设备侧强制的天花板，它们不在这里改，而是在
「高级参数（体感）」页（默认 100，改成 0 就是"那一通道不会有任何输出"——连上却一点感觉都
没有，先看那两行）。会话跑起来后两个通道会一直播一段默认波形，所以强度、上限都是 0 时设备
上的灯也会闪；`Y` 打开这一页自己的日志（和 Socket 页的日志页同一套布局），`A` 启动会话、
`X` 停掉，`B` 返回（返回时会话会一起停）。
