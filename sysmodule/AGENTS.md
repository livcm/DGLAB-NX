...
- **BLE 模式**：直接连接 DG-LAB 设备（Coyote 协议）——**正式入口已实机跑通**（IPC 的
  ...
  协议层。两个前提：**装 exefs 补丁**；**BF 的两个通道强度上限是输出闸门，0 就是那一通道
  不输出**（`BLE_START` 带上限对，运行中改走 `BLE_LIMIT`）。**上限与强度是两组不同的变量**
  （上限是设备强制的天花板、强度是客户端拨的数，各一对 A/B），不要把两者混在一个字段里。
  另外：**"在输出"是靠波形数据成立的**——会话自己会一直播一段默认波形，所以强度与上限都是 0
  时设备上的灯也会闪；不播波形（B0 的波形槽全零）时设备会按官方规则丢掉整个通道的数据，
  什么都看不到，强度也就没有东西可放大。但设备侧一条回包都没有（通知与读应答全哑、
  B1 未验证），所以协议要求的"确认后再改强度"还做不了，按**开环**设计。
