...
1. BLE 模式（sysmodule 直连设备）：**正式入口已实机跑通**（NRO 的 `bluetooth (direct)` 页 +
   ...
   播一段默认波形，所以强度与上限都是 0 时设备上的灯也会闪（手机 App 的实测）；不播波形时
   设备会丢掉整个通道的数据，那就真的什么都看不见（漏掉这一点或上限为 0 的表现都是
   "连上了、有包、就是没感觉"）。**回包方向已查到底**：
   ...
   不要给 btm 留无法完成的连接）。下一步待做：通道强度上限 > 0 的玩法轮（实机确认体感/触屏
   在正式会话路径上真的让设备输出）；把补丁纳入发布产物。
