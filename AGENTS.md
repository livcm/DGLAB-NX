...
1. BLE 模式（sysmodule 直连设备）：**正式入口已实机跑通**（NRO 的 `bluetooth (direct)` 页 +
   ...
   挡下）；**软上限是设备侧的输出闸门，0 就等于设备不会有任何输出**——`BLE_START` 带初值，
   运行中改走 `BLE_LIMIT`（漏掉这一步的表现是"连上了、有包、就是没感觉"）。**回包方向已查到底**：
   ...
   不要给 btm 留无法完成的连接）。下一步待做：软上限 > 0 的玩法轮（实机确认体感/触屏在正式
   会话路径上真的让设备输出）；把补丁纳入发布产物。
